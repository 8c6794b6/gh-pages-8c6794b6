...
Simon Peyton Jones has posted
[a summary](http://www.mail-archive.com/haskell-prime@haskell.org/msg03400.html)
about relationship between type families and functional dependencies.
As well as similarities, there exist difference between functional
...
The situation I met was combination of overlapping instance with
higher rank type, and use of open recursion. Sound bit mixed up. To
make myself clear about this, writing it down in detail.
...
The code I was working was with heterogeneous collection. All of the codes
writen here are heavily inspired from
[typed tagless interpreter by Oleg Kiselyov](http://okmij.org/ftp/tagless-final/course/).
Indeed, this situation came to me when I was doing the exercise
written in one of the code shown in tagless-final courses.
...
And type representations used in our code. This type representation
is simplified version written in GADT, more resembles to
[this version](http://www.comlab.ox.ac.uk/projects/gip/school/tc.hs).
...
Now, suppose that we want to lookup a variable used in `lam`.
What we want to do here is to lookup a value from heterogeneous collection.
...
Function to find variable, with functional dependencies. The result type is
fixed to `(Term S h)`.
...
families works, as shown in `Value` definition of `EnvTF` instance definition.

So far, those things working in functional dependencies are also working in
type families. From here, introducing higher rank type. Now what we want to do
is to deserialize `Tree` data.
...
easy to extend when pattern matching against new serialized
expression. Not going through in detail here, better example could
be found in
[this code](http://okmij.org/ftp/tagless-final/course/SerializeExt.hs).
To get the feeling of deserializer function, showing arguments and
type signature.

> deserializeFD' self t g = undefined

The body of deserializer uses pattern matching, with checking its type. Its
body contains all possible pattern matches, and possibly delegation to `self`
function given as first argument. The actual code could be found in
tagless deserializer link shown above. What concern here is its type
signature.

> deserializeFD' :: forall r. DsrlFD r -> DsrlFD r

We need to tie the knot to use this deserializer. Since variable environment
differs in each recursion, higher rank fixed point combinator is required.
...
deserializer function, let's check that we can tie the knot with higher rank
fixed point combinator, since we already know that need this to support open
recursion.
...
The error message showing that `r`, `g`, and `h ~ Value` differing in each
recursion. In current GHC implementation of type families, it does not allow
us to overlap this with different value. So far I understand, that was the
point. Though, I don't think I fully understand what's happening here, yet.
...
[typecast](haskell-sc-scratch/Scratch/Oleg/typecast/), in this case, used for
solving read-show problem.

...
description: Performance comparison of arrow syntax
---
Recently I read a paper about
["Audio Processing and Sound Synthesis in Haskell"](http://cs.yale.edu/c2/images/uploads/AudioProc-TR.pdf)
which uses `Arrow` and `[Double]` for its building block of signal data.
While reading the document, it shows that using Arrow syntax will make the
...
1. Pure function, no Arrows (_plain_).
2. Arrows, without syntax sugar (_bitter_).
3. Arrows, with syntax sugar (_sweet_).
4. Arrows, with syntax sugar, inlined (_sweet-inline_).
...
It shows that the code written with arrow syntax runs about 9 times slower
than the others, and there's not so much difference between plain function
from arrow codes written without syntax sugar. What's going on with arrow
syntax?
...
Dumped core of `plain`, removed couple type signatures and variable suffixes
for clarity.
...
Though delegation from `bitter` to `bitter1` and `bitter2` exists, from what
we see in the function body of `bitter2`, the guts of its definition remains
same as `plain`.

And then, with arrow syntax. In its body, result of each arrow are bounded to
varialbe with explicit name. Requires `Arrows` LANGUAGE pragma.
...
Excerpt dumped core of `sweet`. We see zipping and unzipping done during the
desugaring of syntax, in `main12` and `$fArrowA3`.
...
Finally, Arrow syntax with INLINE pragma. The only difference between newtype
`A` and `B` is, the use of `INLINE` in function `first`. The function body of
`sweet` and `sweet_inline`, and definitions for `Arrow` class in A and B are
identical.
...
Excerpt dumped core of `sweet_inline`. Note that `zip` function is not used.
Instead, more `Rec` blocks are used. `sweet_inline` runs 2x faster than
`sweet`, a big difference made from single `INLINE` pragma.
...
So, indeed, arrow syntax made the code slow, as written in the paper, mainly
due to those use of tupling and untupling to reference varialbes by names, as
we can find in `sweet` and `sweet_inline`. When arrow is used without syntax
sugar, and there's no tuple packing nor unpacking, its performance is almost
same as pure function variant, as we see in comparison of `plain` and
`bitter`.

When do want to use arrow syntax, even we know that it runs slower? The main
cause of slow down was packing and unpacking of tuples. When we see that these
use of tupes are inevitable, it may show a better looking code. I have not yet
met a practical situation heavily using arrow syntax, which seems valueable

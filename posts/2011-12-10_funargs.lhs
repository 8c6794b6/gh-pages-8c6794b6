...
description: Getting bounded argument names of function
---
...
We want to call the extracter function `funArgs` by writing something like
below, in caller module:
...
`funArgs` cannot extract a function defined in ghci interactively. In this
case, merely calling `error`.
...
When we can acces the source code, parsing the source code to get haskell
representation. Parsing may fail, and in that case, showing error message
with `error` again. There should be nicer way to handle these error cases,
though we just print out the messages, and moving on for now.

...
it motivated me enough to write the implementation in Haskell.

There are couple AVL Tree implementations already exist:
[a package uploaded to hackage](http://hackage.haskell.org/package/AvlTree),
[gist snippet](https://gist.github.com/109729), and
[polymorphic stanamically balanced AVL tree](http://www.okmij.org/ftp/Haskell/types.html#stanamic-AVL).
As for classic data structure and algorithm exercise, decided to write
...
A function to check whether given element is a member of tree or
not. Given element and element of pattern matched node are, again
strictly evaluated.
...
Firstly, comparing with `Leaf` node, which is not needed to rebalance any more.
Then the height from left node and right node are compared, leading to next case.
...
When right node is heavy, we compare the left node and right node
again, and perform the rotations.
...
The right-right case, single rotation swapping the node in the middle
to top and top node to left:

>           LT -> Node (hL1+2) (Node (hL1+1) l1 x1 l2) x2 r2

The right-left case, bringing the bottom element to top, top element
to left, and middle element to right, with reordering each hanging
nodes appropriately.
...
Nothing left to do when nodes already has same height, merely
returning the given node itself.
...
Non-recursive height updating function, used in `EQ` case of `rebalance`:
...
Lessons learned: from above simple implementation, a data structure
with performance close to those provided by standard package could be
implemented. Benchmark show that AVL tree insertion was slightly
slower than insertion in Data.Map, performance of member lookup was
almost identical. Though when there is no need for making own
implemntation, just use the implementation from proven package, in
most case those provided data structures has more features,
Data.Map.Map has useful functions like `insetWith`, `unionWith`, etc.

By the way, during the benchmark I was using `nfIO` instead of
`whnfIO` for a while, resulting to linearly increasing insertion time
for while.